...
export const TableFilter = ErrorBoundaryHoc(() => {
  const { awaitingInventoryOrders, filterDirectionList } = useSelector(state => ({
    ...
    defaultValues: {
      ...
      receiver_code: searchParams.get('receiver_code')
        ? searchParams
            ...
            .map(code => ({ label: code }))
        : null,
      created_date: searchParams.get('created_date_from')
        ? [
  ...
  useEffect(() => {
    ...
    if (awaitingInventoryOrders && orders) {
      const order = awaitingInventoryOrders.find(({ key }) => key === orders);
  ...
  }, [awaitingInventoryOrders, searchParams]);
    ...
    <div className={s.filter}>
      <div className={s.filters}>
        <SelectCustom
          ...
          options={awaitingInventoryOrders?.map(item => ({ label: item.value, value: item.key }))}
      ...
      <div
        className={s.cleaner}
        onClick={() => {
          setSearchParams({ tab: 'pending', page: '1', page_size: '50' }, true);
          reset();
        }}
      >
        <Icon iconId="cleaner" />
