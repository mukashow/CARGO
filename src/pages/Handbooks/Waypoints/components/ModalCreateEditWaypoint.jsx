...
export const ModalCreateEditWaypoint = ErrorBoundaryHoc(
  ({ isOpen, close, mode = 'create', setFieldLabel, expandedPoints }) => {
    const countries = useSelector(state => state.phone.phoneCode);
    const borderPoint = useSelector(state => state.point.borderPoint);
    ...
    const schema = yup.object({
      ...
      from_country: yup.object().required(t('fieldShouldBeFilled')).nullable(),
      to_country: yup.object().required(t('fieldShouldBeFilled')).nullable(),
    });
    ...
    const onSubmit = async values => {
      ...
      try {
        await dispatch(
          mode === 'create'
            ? createBorderPoint(values)
            : updateBorderPoint({ id: borderPoint.id, values })
        ).unwrap();
    ...
    useEffect(() => {
      if (borderPoint && mode === 'edit') {
        const { name_zh_hans, name_en, name_ru, name, country, from_country, to_country } =
          borderPoint;
        setValue('name', name);
        setValue('country', country);
        setValue('from_country', from_country);
        setValue('to_country', to_country);
    ...
    return (
      <Modal isOpen={isOpen} close={() => setCancelConfirm(true)} title={t('borderPoint')}>
