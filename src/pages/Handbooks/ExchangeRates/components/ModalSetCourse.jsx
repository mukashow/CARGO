...
import { ErrorBoundaryHoc, Icon, Input, Modal, ModalAction } from '@/components';
...
export const ModalSetCourse = ErrorBoundaryHoc(({ isOpen, close, fetchExchangeRates }) => {
  ...
  const [backModalOpen, backConfirm, onCancel] = useConfirmNavigate(isOpen && !!data);
  ...
  const onSubmit = async values => {
    try {
      ...
      dispatch(fetchExchangeRaresByCountry(searchParams.get('country')))
        .unwrap()
        .then(setData);
      reset();
      fetchExchangeRates();
  ...
  return (
    <Modal
      ...
      close={() => {
        ...
        setCancelConfirm(true);
        confirmAction.current = close;
      }}
      contentStyle={{ maxWidth: 390, width: '100%' }}
    >
      ...
      {!Object.keys(data || {}).length && (
        <div className={clsx(tableStyle.empty, tableStyle.emptySmall)}>
          <img src="/images/Empty-Box.svg" alt="" />
          <div className={tableStyle.emptyTitle}>{t('exchangeRatesListEmpty')}</div>
        </div>
      )}
      <ModalAction
