...
export const ModalCreateEditCurrency = ErrorBoundaryHoc(({ isOpen, close, mode = 'create' }) => {
  const [cancelConfirm, setCancelConfirm] = useState(false);
  const currency = useSelector(state => state.currency.currencyOne);
  ...
  const prefixName = yup.lazy((_, { path }) => {
    ...
    const schema = yup
      ...
      .nullable();
    if (mode === 'edit' && currency[path] && !i18n.language.match(locale[path])) {
  ...
  const onSubmit = async values => {
    ...
    try {
      await dispatch(
        mode === 'create' ? createCurrency(values) : updateCurrency({ ...values, id: currency.id })
      ).unwrap();
  ...
  useEffect(() => {
    if (currency && mode === 'edit') {
      const { name, name_en, name_ru, name_zh_hans, symbol } = currency;
      setValue('name', name);
  ...
  return (
    <Modal isOpen={isOpen} close={() => setCancelConfirm(true)} title={t('currencyWord')}>
      <form className={s.modal} onSubmit={handleSubmit(onSubmit)}>
        <Input
          name="name"
          register={register}
          errors={errors.name?.message}
          labelText={`${t('currencyName').toLowerCase()}:`}
        ...
        <Input
          ...
          labelText={`${t('currencySymbol').toLowerCase()}:`}
          placeholder={t('modalCreateClientPlaceholder')}
        />
        {i18n.language !== 'zhHans' && (
          <Input
            ...
            labelText={`${t('currencyName').toLowerCase()} ${t('inChinese')}:`}
        ...
        {!i18n.language.match(/ru|ru-RU/) && (
          <Input
            ...
            labelText={`${t('currencyName').toLowerCase()} ${t('inRussian')}:`}
            placeholder={t('enterValueOptional')}
          />
        )}
        {!i18n.language.match(/en|en-US/) && (
          <Input
            ...
            labelText={`${t('currencyName').toLowerCase()} ${t('inEnglish')}:`}
            placeholder={t('enterValueOptional')}
          />
        )}
      ...
      <ModalAction
        title={t(mode === 'create' ? 'toCancelCurrencyCreation' : 'toCancelCurrencyEditing')}
        description={t('currencyDataWillNotBeSaved')}
